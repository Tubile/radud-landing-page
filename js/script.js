/* ===========================================================================
   ...
   =========================================================================== */
const CONFIG = {
  // וואטסאפ — פורמט בינלאומי בלי +.
  whatsapp: "972559400995",
...
function wireContacts() {
  if (CONFIG.whatsapp) {
    ...
    /* Broadcast CTAs (hero / closing / popup) — replace YOUR_NUMBER_HERE placeholders */
    const waBroadcast = `https://wa.me/${CONFIG.whatsapp}?text=${encodeURIComponent(
      CONFIG.whatsappBroadcastText || "עמוק"
    )}`;
    $$("[data-wa-broadcast]").forEach((a) => (a.href = waBroadcast));
  }
  if (CONFIG.email) {
...
function showFormSuccess(form) {
  ...
  /* CTA ראשי — שידור קלף בוקר בוואטסאפ (hero + closing + popup) */
  if (CONFIG.whatsapp) {
    ...
    /* Also sync siblings so no YOUR_NUMBER_HERE remains after any form succeeds */
    $$("[data-wa-broadcast]").forEach((a) => {
      a.href = waHref;
    });
  }
